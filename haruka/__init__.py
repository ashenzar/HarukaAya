...
if CONFIG.spamwatch_api:
    try:
        CONFIG.spamwatch_client = spamwatch.Client(CONFIG.spamwatch_api)
    except Exception as spamwatch_error:
        logging.error(f"Could not initialize SpamWatch client due to a {type(spamwatch_error).__name__}: {spamwatch_error}")
        exit(1)
else:
    logging.warning("SpamWatch API key is missing! Check your config.yml")
